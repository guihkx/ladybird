...
{
    return instruction_names().get(opcode).value_or(ByteString::formatted("<unknown {:#x}>", opcode.value()));
}

Optional<OpCode> instruction_from_name(StringView name)
{
    static HashMap<ByteString, OpCode> reverse;

    if (reverse.is_empty()) {
        for (auto& entry : instruction_names())
            reverse.set(entry.value, entry.key);
    }
...
{
    static HashMap<OpCode, ByteString> map {
...
{
    static HashMap<ByteString, OpCode> map;
    return map;
}
